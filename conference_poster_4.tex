...
\headerbox{GUI}{name=gui, column=1, span=2, below=introduction}{
	\begin{center}
...
\headerbox{Example}{name=example, column=1, span=2, below=gui}{
Along with existing GNU Radio blocks, a complete FM signal processing from receiving to demodulation can be performed live and automatically.

	\centering
	\includegraphics[width=0.9\textwidth]{fm}
	\captionof{figure}{Live FM demodulation}
}
...
\headerbox{Components}{name=components,column=0,below=introduction, bottomaligned=example}{ % This block's bottom aligns with the bottom of the conclusion block
...
Takes messages from the Signal Separator and extracts only the samples belonging to the specified signal in the block parameters. The extracted signals get passed as a complex stream. The input samples can be resampled to satisfy a constand output sample rate.
...
Maintainers of this module:\\[0.1em]

Sebastian Müller\\
KIT Karlsruhe\\
gsenpo@gmail.com\\[0.1em]
